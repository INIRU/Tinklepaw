-- RPC helpers for atomic operations

drop function if exists ensure_user(text);
drop function if exists perform_gacha_draw(text, uuid);

create or replace function ensure_user(p_discord_user_id text)
returns void
language plpgsql
as $$
begin
...
begin
  ...
  if v_item is null then
    -- if forced rarity had no candidates, fall back to any rarity
    with candidates as (
      select
        i.item_id,
        i.name,
