...
declare
  ...
  v_weight_whale numeric := 0.16;
  v_weight_shrimp numeric := 0.64;
  v_weight_ant numeric := 0.20;
...
begin
  ...
  if v_ant_due then
    v_total_weight := v_weight_whale + v_weight_shrimp + v_weight_ant;
    v_roll := random() * v_total_weight;

    if v_roll < v_weight_whale then
      v_actor := 'whale';
    elsif v_roll < (v_weight_whale + v_weight_shrimp) then
      v_actor := 'shrimp';
    else
      v_actor := 'ant';
    end if;
  else
    v_total_weight := v_weight_whale + v_weight_shrimp;
    v_roll := random() * v_total_weight;
    v_actor := case
      when v_roll < v_weight_whale then 'whale'
      else 'shrimp'
    end;
  end if;
  ...
  values (
    ...
    jsonb_build_object(
      ...
      'ant_due', v_ant_due,
      'actor_weight_whale', v_weight_whale,
      'actor_weight_shrimp', v_weight_shrimp,
      'actor_weight_ant', v_weight_ant
    )
  );
