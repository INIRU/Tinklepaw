-- Fix fallback logic in perform_gacha_draw.
-- If selected rarity has no items, try 'R' rarity first, then ANY rarity.
-- Prevents getting SSS items when R items are missing.
...
begin
  ...
  -- [Fallback 1] If no items of selected rarity, try 'R' items
  ...
  -- [Fallback 2] If still null, try ANY items
