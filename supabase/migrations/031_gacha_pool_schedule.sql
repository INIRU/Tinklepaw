-- Add schedule window for gacha pools (limited pickup).
-- Pool should disappear after end_at, but items must never be deleted.
...
alter table gacha_pools
  drop constraint if exists gacha_pools_schedule_check;

alter table gacha_pools
  add constraint gacha_pools_schedule_check
  check (start_at is null or end_at is null or start_at < end_at);

-- Enforce schedule window in draws.
