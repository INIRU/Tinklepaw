alter table nyang.app_config
  add column if not exists lottery_ticket_price integer not null default 500,
  add column if not exists lottery_jackpot_base_points integer not null default 20000,
  add column if not exists lottery_gold_payout_points integer not null default 5000,
  add column if not exists lottery_silver_payout_points integer not null default 1500,
  add column if not exists lottery_bronze_payout_points integer not null default 700,
  add column if not exists lottery_jackpot_pool_points integer not null default 0;
...
begin
  ...
  elseif v_roll < (v_jackpot_rate_pct + v_gold_rate_pct + v_silver_rate_pct + v_bronze_rate_pct) then
    ...
    update app_config
    set
      lottery_jackpot_pool_points = greatest(0, coalesce(lottery_jackpot_pool_points, 0)) + v_ticket_price,
      updated_at = now()
    where id = 1;

    v_jackpot_pool_points := v_jackpot_pool_points + v_ticket_price;
  end if;
