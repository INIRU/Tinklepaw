-- Restore gacha_pool_items join in perform_gacha_draw.
-- This allows pool-specific item configuration (limited pools, etc.)
-- Items must be assigned to the pool via gacha_pool_items to be pulled.
...
begin
  ...
  -- [Fallback 1] If no items of selected rarity in this pool, try 'R' items in this pool
  if v_item is null and v_rarity <> 'R' then
    with candidates as (
      select
        i.item_id,
        i.name,
  ...
  -- [Fallback 2] If still null, try ANY items in this pool
