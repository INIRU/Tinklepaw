-- ======================================
-- 097: Stock Price Pressure System
-- Applies dynamic buy/sell bias based on price proximity to floor/ceiling
-- Target range: 1,000P ~ 15,000P
-- ======================================
...
declare
  ...
  v_pressure_bias numeric := 0;
  v_buy_bias numeric := 0.5;
  v_actor text;
  v_side text;
  v_qty integer;
  v_impact_bps integer;
  v_impact_multiplier numeric := 1.0;
  ...
  -- Price pressure constants
  c_price_floor constant integer := 1000;
  c_price_ceiling constant integer := 15000;
begin
  ...
  -- Push price back toward target range (1,000 ~ 15,000)
  if v_market.current_price >= (c_price_ceiling * 0.85) then
    -- Very high: strong sell pressure
    v_pressure_bias := -0.35;
    v_impact_multiplier := 2.0;
  elsif v_market.current_price >= (c_price_ceiling * 0.70) then
    -- High: moderate sell pressure
    v_pressure_bias := -0.18;
    v_impact_multiplier := 1.4;
  elsif v_market.current_price <= (c_price_floor * 1.3) then
    -- Very low: strong buy pressure
    v_pressure_bias := 0.35;
    v_impact_multiplier := 2.0;
  elsif v_market.current_price <= (c_price_floor * 2.0) then
    -- Low: moderate buy pressure
    v_pressure_bias := 0.18;
    v_impact_multiplier := 1.4;
  else
    -- Normal range: slight linear pressure toward midpoint
    v_pressure_bias := ((c_price_floor + c_price_ceiling) / 2.0 - v_market.current_price)::numeric
                       / (c_price_ceiling - c_price_floor)::numeric * 0.12;
    v_impact_multiplier := 1.0;
  end if;

  v_buy_bias := greatest(0.05, least(0.95, v_base_buy_bias + v_signal_bias + v_pressure_bias));
  ...
  if v_actor = 'ant' then
    -- Ant respects pressure: only buy when bias > 0.4, else sell
    if v_buy_bias > 0.4 then
      v_side := 'buy';
    else
      v_side := 'sell';
    end if;
  ...
  v_impact_bps := ceil(v_impact_bps * v_impact_multiplier)::integer;
  ...
  if v_side = 'buy' then
    v_price_after := least(c_price_ceiling + 1000, v_price_before + v_price_delta);
  else
    v_price_after := greatest(c_price_floor / 2, v_price_before - v_price_delta);
  end if;
  ...
  values (
    ...
    jsonb_build_object(
      ...
      'pressure_bias', v_pressure_bias,
      'impact_multiplier', v_impact_multiplier,
