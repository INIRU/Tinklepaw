alter table nyang.app_config
  add column if not exists daily_chest_legendary_rate_pct double precision not null default 3,
  add column if not exists daily_chest_epic_rate_pct double precision not null default 15,
  add column if not exists daily_chest_rare_rate_pct double precision not null default 30,
  add column if not exists daily_chest_common_min_points integer not null default 40,
  add column if not exists daily_chest_common_max_points integer not null default 110,
  add column if not exists daily_chest_rare_min_points integer not null default 90,
  add column if not exists daily_chest_rare_max_points integer not null default 200,
  add column if not exists daily_chest_epic_min_points integer not null default 180,
  add column if not exists daily_chest_epic_max_points integer not null default 360,
  add column if not exists daily_chest_legendary_min_points integer not null default 340,
  add column if not exists daily_chest_legendary_max_points integer not null default 620,
  add column if not exists daily_chest_item_drop_rate_pct double precision not null default 12;
...
declare
  ...
  v_item_drop_rate_pct double precision := 12;
...
begin
  ...
  if found then
    ...
    v_item_drop_rate_pct := greatest(0.0, least(100.0, coalesce(v_cfg.daily_chest_item_drop_rate_pct, 12)));
  ...
  v_roll := random() * 100.0;
  if v_roll < v_legendary_rate_pct then
    v_tier := 'legendary';
    v_points := v_legendary_min + floor(random() * (v_legendary_max - v_legendary_min + 1))::integer;
  elseif v_roll < (v_legendary_rate_pct + v_epic_rate_pct) then
    v_tier := 'epic';
    v_points := v_epic_min + floor(random() * (v_epic_max - v_epic_min + 1))::integer;
  elseif v_roll < (v_legendary_rate_pct + v_epic_rate_pct + v_rare_rate_pct) then
    v_tier := 'rare';
  ...
  if random() * 100.0 < v_item_drop_rate_pct then
