-- Fix perform_gacha_draw to select items directly from 'items' table
-- instead of relying on 'gacha_pool_items' junction table.
-- This allows items to be pulled without explicit pool assignment.
...
begin
  ...
  -- [CHANGED] Removed gacha_pool_items join. Select from items directly.
  -- Rarity matches v_rarity. Uniform probability among items of same rarity.
  with candidates as (
    select
      i.item_id,
      i.name,
  ...
  if v_item is null then
    -- Fallback: if no items of v_rarity exist, pick ANY active item (ignore rarity)
    with candidates as (
      select
        i.item_id,
        i.name,
