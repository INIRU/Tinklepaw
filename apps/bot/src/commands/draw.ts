import { SlashCommandBuilder, EmbedBuilder, ActionRowBuilder, ButtonBuilder, ButtonStyle, Message, type MessageComponentInteraction, type Collection } from 'discord.js';
...
export async function triggerGachaUI(context: ChatInputCommandInteraction | Message) {
  ...
  const { data: pools } = await ctx.supabase
    .from('gacha_pools')
    .select('pool_id, name, kind, banner_image_url, cost_points, rate_r, rate_s, rate_ss, rate_sss, pity_threshold, pity_rarity')
    .eq('is_active', true)
  ...
  const showPool = async (index: number, isEdit = false): Promise<Message | null> => {
    ...
    (items ?? []).forEach((item) => {
      if (item.rarity in itemsByRarity) {
        const roleMention = item.discord_role_id ? `<@&${item.discord_role_id}>` : item.name;
        itemsByRarity[item.rarity as keyof typeof itemsByRarity].push(roleMention);
      }
    });
    ...
    const embedDescription = (botConfig.gacha_embed_description as string || '현재 포인트: **{points}p**\n1회 뽑기 비용: **{cost1}p**\n10회 뽑기 비용: **{cost10}p**{pity}\n\n**확률표 & 획득 가능 역할**\n{rarityDisplay}')
    ...
    const embed = new EmbedBuilder()
      ...
      .setImage(pool.banner_image_url || 'https://via.placeholder.com/800x300?text=Gacha+Banner')
    ...
    if (isEdit) {
      ...
      if (context instanceof Message) {
        ...
        const reply = await context.reply({ embeds: [embed], components: [buttons], withResponse: true });
  ...
  const collector = ('createMessageComponentCollector' in initialReply) 
  ...
  collector.on('collect', async (buttonInteraction: MessageComponentInteraction) => {
    if (buttonInteraction.customId === 'prev') {
      ...
      await buttonInteraction.deferUpdate();
      await showPool(currentIndex, true);
    } else if (buttonInteraction.customId === 'draw1' || buttonInteraction.customId === 'draw10') {
      const selectedPool = pools[currentIndex];
      const drawCount = buttonInteraction.customId === 'draw10' ? 10 : 1;

      const processingTitle = botConfig.gacha_processing_title.replace('{drawCount}', drawCount.toString());
      const processingDesc = botConfig.gacha_processing_description.replace('{drawCount}', drawCount.toString());

      await buttonInteraction.update({ 
        embeds: [new EmbedBuilder().setTitle(processingTitle).setDescription(processingDesc).setColor(parseColor(botConfig.gacha_embed_color))], 
        components: [] 
      });
      collector.stop();

      const results: Array<{ name: string; rarity: 'R' | 'S' | 'SS' | 'SSS'; refund_points?: number; discord_role_id?: string | null }> = [];
      const errors: string[] = [];
      ...
      if (results.length === 0) {
        ...
        await buttonInteraction.editReply({ embeds: [], components: [], content: `뽑기 실패: 결과가 없습니다.${errorSummary}` });
        return;
      }

      const resultsByRarity = { SSS: [] as typeof results, SS: [] as typeof results, S: [] as typeof results, R: [] as typeof results };
      results.forEach((r) => { if (r.rarity in resultsByRarity) resultsByRarity[r.rarity as keyof typeof resultsByRarity].push(r); });
      ...
      (['SSS', 'SS', 'S', 'R'] as const).forEach((rarity) => {
        ...
        if (items.length > 0) {
          const itemNames = items.map((item) => {
            const displayName = item.discord_role_id ? `<@&${item.discord_role_id}>` : `**${item.name}**`;
      ...
      const resultImage = await generateGachaResultImage(results, selectedPool.name);
      const resultTitle = botConfig.gacha_result_title.replace('{drawCount}', drawCount.toString());
      const resultEmbed = new EmbedBuilder().setTitle(resultTitle).setDescription(resultLines.join('\n\n') || '결과 없음').setImage('attachment://gacha-result.png').setColor(parseColor(botConfig.gacha_embed_color));

      await buttonInteraction.editReply({ embeds: [resultEmbed], components: [], files: [resultImage] });
    }
  });
  ...
  collector.on('end', (collected: Collection<string, MessageComponentInteraction>) => {
    if (collected.size === 0) {
      if (context instanceof Message) {
        initialReply.edit({ embeds: [], components: [], content: '⏱️ 시간이 초과되었습니다. 다시 `/뽑기` 명령어를 사용해주세요.' }).catch(() => {});
      } else {
        context.editReply({ embeds: [], components: [], content: '⏱️ 시간이 초과되었습니다. 다시 `/뽑기` 명령어를 사용해주세요.' }).catch(() => {});
      }
    }
  });
}
